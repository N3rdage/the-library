...
- [Mobile companion app arc — six PRs, ~20 wall-clock hours](retro_mobile_companion_arc.md) — `BookTracker.Mobile` (.NET MAUI Android) shipped from zero across PRs #211–#218. Headline lesson: speed came from the Web app having already paid every design tax (catalog DTOs, debounce timing, AAD wiring, alias rollup, JSON enum convention). Bookshop and mobile are two solutions for two problems. Surprises included the silent AAD broker-intercept hang and the sqlite-net-pcl path-as-literal-filename for test isolation
- [Add/View/Edit polish arc — six PRs decommissioning /edit](retro_add_view_edit_polish_arc.md) — PRs #239–#245. Status chip fix → unified Genre picker → collection-mode Genre toggle → attach-existing dialog → keyboard-first existing-Work autocomplete on Add rows → /edit decommission. Net delete: ~960 LOC (Edit.razor + BookEditViewModel). Three durable patterns: the "Ask, but allow free-text" UX shape (Drew steered to it twice); the MudAutocomplete + capture-phase JS recipe for suppressing auto-pick; the Blazor dialog VM Transient trap. One accepted papercut: MudAutocomplete's portaled popover stays open after free-text Enter (three close-it-for-them attempts failed; Esc dismisses)
- [Series-order reconciliation arc — 3 PRs + bookkeeping](retro_series_order_arc.md) — PRs #329 (web `SeriesOrderDisplay` + floor-and-display parser), #330 (mobile snapshot + cache sync), #335 (#53c retire Group-by-Series). Reconciled the Series TODO cluster: #3 deleted (dup), #16 closed as shipped-but-not-moved drift, #14 shipped, #13 deferred, #53 shipped. Headline lesson: the overloaded-column trap — flooring "4.5"→int `SeriesOrder` silently broke gap detection because every existing reader assumed the int was a real numbered volume (fix: one shared `SeriesSlots.OccupiesNumberedSlot` predicate). Plus: removing Group-by-Series left the replacement path unreachable from the default view; and a catalog-refresh red-herring where two confident theories each died to one Drew fact. High-effort review caught a regression on every PR; the PR2 review also produced the "present findings then wait" feedback
- [Bookshelf UI redesign arc — ~7 PRs, Shell + tabs + motion](retro_bookshelf_redesign_arc.md) — implemented `docs/BOOKSHELF-UI-REDESIGN.md` (a separate design agent's brief): foundation tokens (#337) → Shell + 3 bottom tabs (#338) → status sheet + `SyncService` (#339) → AuthorWorks (#340) → unified Find + ResultPage (#341) → inline ZXing scanner, ScanPage deleted (#342) → arc-review fixes (#343) → Wishlist sort/grouping + Gaps progress/pills → motion layer (§9). Closed TODO #37 (palette + dark coverage); spawned #54 (online "do better when online" layer). Durable lessons: name what the offline cache *can't* answer and arc it separately; defer the high-effort review to ARC END for multi-PR arcs (mid-arc reviews flag the next PR's work); a native control's size obeys the layout rect, not `HeightRequest` (the ZXing inline-camera saga); `--no-incremental` to actually see warnings (caught 5 self-introduced CS0618s → TD-11). Treat a brief's code as pseudocode — keep its decisions, re-derive its mechanics
- [Series-on-Book arc — 3 PRs, expand→cutover→contract](retro_series_on_book_arc.md) — moved series membership Work→Book (TODO #56, #401 expand / #402 cutover / PR3 contract) after Drew dogfooded the multi-Work short-story-collection case ("a series groups works, not books"). Headline surprise: the model was *already half-migrated* — the mobile cache + wire format stored series at book grain via a lossy "first-Work" projection, so the cutover **deleted a workaround** and mobile was free. Durable lessons: a grep's file list is a checklist (PR2's grep surfaced the AI/series-match files, I acted on one → round-1 review caught 3 silent-failure bugs); the contract column-drop is a free reference-finder (compiler enumerated every straggler incl. a DeleteSeries test missed in PR2); required-args-over-defaulted on a state-reconciling command (`UpdateBookDetails` silent-series-clear footgun); re-review converges (round-1 bugs → round-2 polish → stop), and the verifier refutes false positives (the EF-translation scare died to `DuplicateDetectionService` precedent). Spawned TD-20
