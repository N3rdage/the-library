...
- **11 strong catch-up candidates** (9 from corpus mine + 2 added 2026-05-22 from the genre-cleanup dogfood arc).
...
- **"Mobile took 20 hours because the Web took everything else"** — `retros/retro_mobile_companion_arc.md`. Six PRs across ~20 wall-clock hours (8h active) shipped a working AAD-secured Android companion app from zero: shared DTOs → AAD runbook → MAUI skeleton → SQLite cache → barcode scan + library lookup → letterbox polish. The headline isn't the speed; it's *why* it was fast. Every design choice was already paid for elsewhere — catalog DTO existed (Web's PWA), debounce timing decided (`/bookshop` barcode-scanner.js), AAD already secured the API (audience-mode toggle, not redesign), JSON enum convention chosen, alias rollup query written. The mobile arc was composition, not creation. Companion framing: "Bookshop was the dress rehearsal; mobile was the show" — `/bookshop` shipping as a zero-Blazor-interactivity page that *happened* to work via SW + cache taught the codebase what the offline-mobile-lookup workflow wanted, then mobile delivered it in the right container without inventing anything new. Two specific diagnostic war stories worth landing: (a) AAD broker hang from URL-encoded redirect URI mismatching Android's intent-decoded match — silent, no exception, no logcat — fix was `.WithBroker(false)` + literal `+`/`=`; (b) PR4 NRE in the success-message string interpolation, not in the populate path defensive-null work, because the API hadn't been redeployed yet — diagnosis lesson on capturing the throw frame when the user-visible message is generic. Audience: solo builders considering a mobile/second-app project (the time estimate isn't "build a mobile app," it's "compose your existing decisions into a mobile container" — multi-week first time, long weekend second). Drew flagged the speed angle in-chat: "look what we shipped in under 24 hours feels like a really good story." A "how is the mobile app working in real life" follow-up is on Drew's natural radar for later.
