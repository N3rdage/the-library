...
| C11 | An aggregate may **own a shared many-to-many relationship and self-manage its lifecycle by ref count**. `Work` owns Book↔Work: it's born attached to its first book (the `Create` factory), gains/loses books via `AppearsIn`/`RemoveFrom`, and is orphaned at ref count 0. Route *all* relationship mutations through the owning aggregate's methods (don't `book.Works.Add` in one place and `work.AppearsIn` in another). The orphan-delete is **signalled** by the aggregate (`RemoveFrom` returns "now orphaned") but **executed** by the handler (`db.Remove`) — the same split as a soft-delete. Relationship soft-outcomes (already-attached, target gone) stay **nullable returns**, distinct from a `NotFoundException` (which is for a mutated *root* that's gone). *(Added after the PR2 review — the Work feature established this; PR3/Series and later aggregates follow it.)* |
| C12 | **Construct aggregates + their children through factories/methods, never raw `new` in a handler.** A handler that creates an aggregate or adds a child routes through the seam that owns the invariant — `Work.Create`, `book.AddEdition` (which seeds the mandatory first `Copy` and normalises the ISBN) — not `new Work { … }` / `new Edition { Copies = [new Copy …] }`. Raw construction in a handler silently *forks* the invariant: the next change to the aggregate's defaults or guards won't reach that site, and the next reader can't tell whether bypassing the factory was deliberate. This is the corollary of C6 (invariants live on the aggregate) for the **creation** path. *(Added after the PR4 review — the wishlist promotion hand-rolled an Edition+Copy instead of `book.AddEdition`.)* |
